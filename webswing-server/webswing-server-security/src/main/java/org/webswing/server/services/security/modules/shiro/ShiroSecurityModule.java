...
public class ShiroSecurityModule extends AbstractUserPasswordSecurityModule<ShiroSecurityModuleConfig> {
    ...
    */
    ...
    static {
        try {
            getAuthorizationInfoMethod = AuthorizingRealm.class.getDeclaredMethod("getAuthorizationInfo", PrincipalCollection.class);
            getAuthorizationInfoMethod.setAccessible(true);
        } catch (NoSuchMethodException e) {
            log.error("Failed to setup dynamic role discovery: getAuthorizationInfo method not found.", e);
        }
    }
    ...
    private void internalInit(String iniFileName) {
        try {
            ...
            */
            ...
            log.info("Shiro configuration (re)loaded from {}. Dynamic role discovery active.", iniFileName);
        } catch (Exception ex) {
            log.error("Failed to load Shiro configuration from {}. Previous state remains active.", iniFileName, ex);
        }
    }
    ...
    public AuthenticatedWebswingUser verifyUserPassword(String user, String password) throws WebswingAuthenticationException {
        ...
        if (currentMgr != null) {
            ...
            try {
                ...
                throw new WebswingAuthenticationException("Unknown user '" + user + "'. Please check your username.",
                ...
                throw new WebswingAuthenticationException("Account '" + user + "' is locked. Please contact your administrator.",
                                                          WebswingAuthenticationException.NO_ACCESS);
            } catch (DisabledAccountException e) {
                log.warn("Authentication failed for user '{}': account disabled.", user);
                throw new WebswingAuthenticationException("Account '" + user + "' is disabled. Please contact your administrator.",
                                                          WebswingAuthenticationException.NO_ACCESS);
            } catch (ExcessiveAttemptsException e) {
                log.warn("Authentication failed for user '{}': too many attempts.", user);
                throw new WebswingAuthenticationException("Too many failed login attempts for '" + user + "'. Please try again later.",
                ...
                throw new WebswingAuthenticationException("Authentication failed for '" + user + "': " + detail,
                                                          WebswingAuthenticationException.FAILED_TO_AUTHENTICATE);
            }
        ...
        throw new WebswingAuthenticationException("Security Manager not initialized.", WebswingAuthenticationException.CONFIG_ERROR);
    }
    ...
    String deobfuscate(String encoded) {
        try {
            ...
            if (decoded.length() < 8 || !decoded.matches(".*\\d{8}$")) return encoded;
            ...
            for (int i = 0; i < front.length(); i++) indices.add(i);
            ...
            for (int i = 0; i < indices.size(); i++) unshuffled[indices.get(i)] = shuffled[i];
    ...
    static class Mulberry32 {
        private int state;
        public Mulberry32(int seed) { this.state = seed; }
        public float next() {
