 ...
 * anymore.
 * 
 * A {@link HashMap} doesn't help here since it will keep hard references for key and value objects.
 * A {@link WeakHashMap} doesn't either, because it keeps weak references to the key objects, but we
 * want to track the value objects.
 * 
 * This implementation of a Map uses a {@link WeakReference} to the value objects. Once the garbage
 * collector decides it wants to finalize a value object, it will be removed from the map
 * automatically.
 * 
...
public class WeakValueHashMap<K, V> extends AbstractMap<K, V> {
  ...
  public Set<Map.Entry<K, V>> entrySet() {
    ...
    for (Map.Entry<K, WeakValue<V>> entry : references.entrySet()) {
      entries.add(
          new AbstractMap.SimpleEntry<K, V>(entry.getKey(), getReferenceValue(entry.getValue())));
    }
    return entries;
  }
